...
export async function detectIdes(projectPath = process.cwd()) {
  ...
  if (await fs.pathExists(path.join(projectPath, '.claude'))) {
    ides.push({
      ...
      priority: IDE_PRIORITIES['VSCode / Claude Code']
    });
  }

  // Cursor — check .cursor/ directory (modern) or .cursorrules (legacy)
  if (await fs.pathExists(path.join(projectPath, '.cursor')) ||
      await fs.pathExists(path.join(projectPath, '.cursorrules'))) {
    ides.push({
      name: 'Cursor',
      configDir: '.cursor',
  ...
  // Codex CLI — check .agents/ (OpenAI convention) or .codex/
  if (await fs.pathExists(path.join(projectPath, '.agents')) ||
      await fs.pathExists(path.join(projectPath, '.codex'))) {
    ides.push({
      name: 'Codex CLI',
      configDir: '.agents',
      instructionsFile: '.codex/rules.md',
