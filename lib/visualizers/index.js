...
export async function generateVisualization(options = {}) {
  ...
  try {
    ...
    const currentIde = await _detectCurrentIdeForFilter();
    ...
    let html = template
      ...
      .split('{{SIDEBAR_STATS}}').join(renderSidebarStats(stats))
      .split('{{IDE_TABS}}').join(renderIdeTabs(effectiveIdeFilter))
