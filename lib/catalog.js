 ...
 */
const DEFAULT_CATALOG_PATH = path.join(process.cwd(), 'catalog.json');
...
export async function loadCatalog(catalogPath = DEFAULT_CATALOG_PATH) {
  try {
    ...
    if (error.message.includes('Catalog validation failed') || error.message.includes('Catalog file not found')) {
      throw error;
    }
    throw new Error(`Failed to load catalog from ${catalogPath}: ${error.message}`);
  }
}
